/**
 * Задание: метод принимает отображение в котором содержатся
 * пары ключ(число) - значение(строка), при этом строка - это имя пользователя
 * (в отображении они представлены пустой строкой), ключ - его id.
 * Также в качестве аргумента метод принимает список пользователей.
 * Необходимо реализовать метод, который произведет ассоциацию
 * id и name пользователя в отображении. При этом:
 *
